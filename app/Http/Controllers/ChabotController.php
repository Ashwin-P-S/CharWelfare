...
class BotManController extends Controller
{
    ...
    public function showAllAvailableSchemes($botman): void
    {
        ...
        foreach ($this->schemes as $scheme) {
            $button = Button::create(strtoupper($scheme->disability))
     ...
     */
    public function classify($message): string
    {
        return $this->classifier->predict([$message])[0];
    }
    ...
    public function detectLanguageAndProcessInformation($message, $botman): void
    {
        ...
        if (preg_match("/(.*)\s+schemes?/i", $message)) {
            ...
            $message = $this->classify($message);

            // Greeting!
            if (preg_match("/Hello|Hi|hi|hello|greeting/", $message)) {
                $this->showMessage($botman, "Hello!");
                $this->showInformation($botman);
            }

            // Get all available Schemes
            else if ($message == "list" || preg_match('/list/i', $message))
            ...
            // Show some Gratitude!
            else if (preg_match("/thank|you|gratitude/i", $message)) {
